    ...
    internal class Program
    {
        ...
        static void TestDir(string inPath)
        {
            ...
            foreach (var cDir in cdDirInfo.GetDirectories())   // Test all Files INFO
            {
                ...
                crDeep = crDeep + 1;
                TestDirFiles(cDir.FullName); //Directory
                crDeep = crDeep - 1;
        ...
        static bool CheckFileTime(FileInfo crFile, byte flPrcMode = 0)
        {
            TimeSpan diff;
            DateTime flTimeVal;
            if (flPrcMode > 0)
        ...
        static void TestDirFiles(string inPath)
        {
            ...
            foreach (var cFl in cDirInfo.GetFiles())   // Test all Files INFO
            {
                //Directory
                //TotalFileSize = TotalFileSize + cFl.Length;
                //Console.WriteLine("{0}> {1}.LastWriteTime={2}.|", Promt, cFl.Name, cFl.LastAccessTime .LastWriteTime);

                //Console.WriteLine("{0}!{3}> {1}.LastAccessTime={2}.|", Promt, cFl.Name, cFl.LastAccessTime, crDeep);
                if (CheckFileTime(cFl))
                {
